...
{#if form?.message}
  <p class="mb-3 rounded border p-2 {form.success ? 'border-green-400 bg-green-50' : 'border-red-400 bg-red-50'}">
    {form.message}
  </p>
{/if}
...
<div>
  <label for="gradeSystem" class="block mb-1">Sistema de grado</label>
  <select
    id="gradeSystem"
    name="gradeSystem"
...
<div>
  <label for="value" class="block mb-1">Valor</label>
  ...
  {:else}
    <input id="value" name="value" class="border px-2 py-1 bg-gray-100" value="" disabled />
  {/if}
</div>
  ...
  <div>
    <label for="difficultyLevel" class="block mb-1">Dificultad percibida</label>
    <select id="difficultyLevel" name="difficultyLevel" bind:value={difficultyLevel} required class="border px-2 py-1">
